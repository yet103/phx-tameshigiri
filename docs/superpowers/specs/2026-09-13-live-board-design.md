...
| 表示するもの | コート、今の選手（順番・名前）、技①②③と各太刀の 未／成功／失敗、補正点、行の得点、全体補正、合計、確定済みか、**タイマー** |
...
- 表示（上から）:
  ...
  3. 表のフッタに全体補正（`totalAdjust` が 0 以外のときだけ「全体補正 +n」）。合計は保存済みの `score`（行の合計＋全体補正。順位表と同じ値）を大きく。`confirmed` なら得点と合計を `--score-confirmed` の青にし、「確定」の札を出す。
  4. 表の枠は行数ぶんの高さにし、残りは空ける（`?bg=transparent` で映像の上に重ねたとき、空いた部分が透ける）。
...
3. A と B の2つを並べる。採点端末で選手を切り替えると 2〜3 秒で追随する。配信に映すコートの端末では、採点画面のコートを必ずそのコートに選んでおく（全コート表示のまま別コートへ移ると前のコートのボードが取り残される）。共有リンクを作り直したらブラウザソースを再読込する。
