...
- 認証切れのときは `beforeunload` の離脱確認を出さない（再読み込みが復旧手段なので止めない。未送信分は localStorage に残る）。バナーの「今すぐ再試行」は認証切れ中は隠す（押しても 401 が返るだけ）。
...
- `deploy.sh` に `set -e` を足す。`.env` が無くて compose が起動を拒否したのに「✅ 完了」と出て、**古い無認証コンテナが `restart: always` で動き続ける**のを防ぐ（2026-09-14 レビューで追加）。サーバー側で `.env` を一度作れば以降のデプロイに引き継がれる。
