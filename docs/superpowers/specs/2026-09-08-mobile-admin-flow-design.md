...
| 技の選び方 | **①②③ の枠ごとにタップして選び直す**（ボトムシートは常に1枠を編集、配点を横に表示、同じ技を複数の枠に入れてよい） |
...
| ファイル | 責務 | 依存 |
|---|---|---|
| `techpicker.js`（新規） | 技のボトムシート選択。選択状態（常に3枠・枠ごとに選び直せる。穴は詰めない。同じ技を複数の枠に入れてよい）を純粋関数で持ち、シートの描画とチップの描画を提供する | `Api`（技術リスト取得） |
...
シートは `position: fixed` の下部パネルで、リストは縦スクロール。シートは常に①②③のうち1つの枠だけを編集するモードで開く。一覧から技をタップするとその枠だけが差し替わってシートが閉じる（他の枠には触れない）。同じ技を複数の枠に入れてよい（重複を許す）。枠を空にするには「（この枠を空にする）」をタップする。
...
16. `TechPicker.setSlot`：指定した枠（0..2）だけを差し替える／空文字を渡すとその枠を空にする／範囲外の index は state をそのまま返す／同じ技を複数の枠に入れてよい（重複を許す）
...
- 「保存して次を追加」で3人続けて登録し、コート・性別・新人が保たれる
