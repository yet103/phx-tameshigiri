...
## 追補: 備考の文例（2026-09-20 追加要望）

「備考の選択肢に、『間合い確認・素振りをしたので』など、文例を選択できるように」への対応。採点画面の備考欄の右に「文例」ボタンを置き、押すと文例の一覧（ボトムシート風のリスト、44px 以上のタップ目標）が開く。選ぶと備考の末尾に追記する（既に文字があれば「、」で区切る。200 文字を超える分は切って alert）。追記後は `change` と同じ保存経路（`onNoteChange`）を通す。採点できない状態（`scoringOpen()` が偽）では文例ボタンも無効。

文例は `data.js` の `NOTE_PRESETS`（配列。順序どおりに並べる）に置き、公式ルールの失敗・無効の理由から:
...
大会ごとの編集は行わない（固定リスト。将来 `settings` に持たせる余地は残す）。ヘルプ「採点画面の見方」に 1 文。`test.html` は `NOTE_PRESETS` が配列で空でないこと、備考の追記の純粋関数 `Scoring.appendNote(current, preset)`（「、」区切り・200 文字で切る・重複した同じ文例は足さない）を固定する。
