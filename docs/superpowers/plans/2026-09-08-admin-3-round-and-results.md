...
3. 計画2 完了時点は **234 passed**。本計画で増えるのは（レビューでの追加分を含め）、最終的に **261 passed, 0 failed** になる
...
Api.generateNextRound(eventId, force)
  // → { success, created, skipped, existingCount, untrackedCount, unassignedCount }
  // | { blocked: true, reason: 'unscored'|'exists', unscoredCount, existingCount, untrackedCount, unassignedCount }
  // | null
...
Courts.compareOrder(a, b) / nextRoundConflictMessage(result, fixHint) / nextRoundResultMessage(result)
```
...
Admin.closeAllSheets()          // 開いているシートを全部閉じる。ハッシュ遷移時に applyRoute が呼ぶ
...
Expected: `Result: 261 passed, 0 failed`
...
| Task 7 完了 | **261 passed, 0 failed** |
| Task 10 完了 | **261 passed, 0 failed** |

**この計画の完了条件は `Result: 261 passed, 0 failed`。**
